...
planet * initPlanetFile(){
	...
	fscanf(data1,"%f", &p->mass);
	fscanf(data1,"%f%f%f", &p->pos.x,&p->pos.y,&p->pos.z);
	fscanf(data1,"%f%f%f", &p->vel.x,&p->vel.y,&p->vel.z);
	return p;
}
...
void newData(planet p){
	fprintf(data2, "%f\t", p.mass);
	fprintf(data2, "%f\t%f\t%f\t", p.pos.x, p.pos.y, p.pos.z);
